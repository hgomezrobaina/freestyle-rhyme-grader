...
export function mapBattleDetailToBattle(detail: BattleDetailResponse): Battle {
  ...
  const maxRound = Math.max(...detail.verses.map((v) => v.round_number ?? 1), 1)
  ...
  const rhymes = detail.verses.map((verse) => {
    ...
    const ratings: RhymeRating = metric
      ? {
          ...
          respuesta: Math.min(5, metric.internal_rhymes_count > 0 ? Math.min(metric.internal_rhymes_count, 5) : 0),
        }
